...
class NotificationType {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "NotificationType[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "NotificationType[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static Map<String, List<NotificationType>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = NotificationType.listFromJson(
          entry.value,
          growable: growable,
        );
      }
