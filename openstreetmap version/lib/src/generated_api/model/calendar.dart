...
class Calendar {
  ...
  @override
  String toString() =>
      'Calendar[id=$id, name=$name, data=$data, attributes=$attributes]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "Calendar[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "Calendar[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Calendar> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<Calendar>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = Calendar.listFromJson(
          entry.value,
          growable: growable,
        );
      }
